...
int marcarLinea(coordenada sentido, coordenada posicionPrevia, int jugador) {
  ...
  while (tablero[posicionPrevia.y - 1][posicionPrevia.x - 1] ==
...
// Revisa una línea, si encuntra otra ficha del jugador en el trayecto devuelve
// verdadero
//  de lo contrario devuelve falso
bool revisarLinea(coordenada sentido, coordenada posicionPrevia, int jugador) {

  while (true) {
    if (tablero[posicionPrevia.y - 1][posicionPrevia.x - 1] == jugador) {
      return true;
    }
    posicionPrevia.x += sentido.x;
    posicionPrevia.y += sentido.y;
...
bool chequearCercanias(coordenada posicion, int jugador) {
  ...
  for (int i = 0; i < 8; i++) {
    ...
    if (!fueraDeRango(posicionVecina)) {
      if ((tablero[posicionVecina.y - 1][posicionVecina.x - 1] ==
           siguienteJugador(jugador)) &&
